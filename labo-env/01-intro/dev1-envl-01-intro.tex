...
\begin{coltbox}{Conseils}
	...
	\item 
		Vous pouvez \emph{coopérer} avec vos condisciples 
		mais nous vous demandons de ne \emph{pas copier} les réponses. 
		Si vous voulez progresser, \emph{chercher} la réponse est plus important que de la trouver. 
	\item 
		N'hésitez pas à \emph{montrer votre travail} à votre professeur.
	\item 
		N'hésitez pas à \emph{poser des questions} 
		si vous n'avez pas bien compris ce qu'on vous demande.
	\item 
		\emph{Prenez des notes} ! 
		Ce que vous allez apprendre aujourd'hui 
		vous servira les semaines prochaines 
		mais vous en aurez oublié une grande partie si vous ne notez rien. 
		Le plus pratique est probablement d'annoter la version \emph{papier}. 
	\end{itemize}
\end{coltbox}
\vfill
...
%====================
	...
	%====================
		...
		nous allons, pour des raisons pédago\-gi\-ques, 
		vous apprendre à utiliser \name{GNU/Linux} en mode console.

		\begin{theorie}{Le mode console}
			En mode console%
			\footnote{%
				On parle aussi de mode \emph{commande} ou encore \emph{terminal}.
			}, 
			nous n'allons pas utiliser des fenêtres et une souris 
			mais nous allons donner (\emph{écrire}) des commandes au système.
		...
		\emph{C'est le principal objectif de ces TDs !}
	...
	%=====================
		...
		\begin{infobox}
			...
			\item[Votre login (username)]
				Un \texttt{'g'} \emph{minuscule} suivi de votre numéro d'étudiant
				(p. ex.: \samp{g52010}).\\
				\textbf{Note} Pour \name{GNU/Linux}, les minuscules et les majuscules 
				sont différentes.
			\item[Votre mot de passe]
				Le même que votre \emph{mot de passe initial sous \name{Windows}}%
				\footnote{%
		...
		\item 
			Cliquez sur \og \textit{Open} \fg ; 
			la connexion se fait ! 
			S'il vous présente une boite de message avec un \og \textit{Security Alert} \fg, 
			cliquez sur \og \textit{Yes} \fg\ en toute confiance.			
		\item 
			Identifiez-vous !
			\begin{steps}
			\item 
...
\newpage
	%====================
	\section{Dossier personnel et dossier courant}
	%=====================
		...
		\begin{theorie}{Fichiers et dossiers}
			...
			\begin{itemize}
			\item 
				Un \emph{fichier} contient de l'information : 
				un code \name{Java}, un exécutable, un texte, etc.
			\item 
				Un \emph{dossier} contient des fichiers. 
				Il sert à organiser/regrouper les nombreux fichiers du disque. 
				On parle aussi de \emph{répertoire}.
			\item 
				Un dossier peut aussi contenir des dossiers qui, à leur tour, 
				peuvent contenir des fichiers/dossiers qui etc.
				On a une organisation \emph{hiérarchique} des fichiers.
			\end{itemize}
		\end{theorie}

		\begin{theorie}{Le répertoire personnel}
			Sur \name{GNU/Linux}, chaque utilisateur dispose 
			d'un \emph{dossier personnel}
			(on dit aussi sa \textit{home}). 
			C'est dans ce dossier qu'il peut créer des fichiers (et des dossiers). 
			Par convention, ce dossier porte le même nom que le \textit{login} 
			de l'utilisateur%
			\footnote{%
		...
		\begin{theorie}{Le dossier courant}
			À tout moment,
			on se trouve dans un dossier précis, le \emph{dossier courant}. 
			Au départ, lorsqu'on se connecte, c'est le \emph{répertoire personnel}.
		\end{theorie}

	%====================
	\section{Contenu d'un dossier ou d'un fichier}
	%=====================
...
\newpage
	...
	%====================
	\section{Structure d'une commande}
	%=====================
		...
		\begin{Experience}{La casse} 
			...
			\begin{infobox}
				En \name{GNU/Linux}, 
				\emph{les majuscules et les minuscules n'ont pas le même sens}.
			\end{infobox}
		\end{Experience}	
		...
		\begin{Experience}{Les espaces} 
			...
			\emph{Les espaces ont de l'importance}.
		\end{Experience}				
	...
	%====================
	\section{Modification du mot de passe sous GNU/Linux}
	%=====================
	...
	%=====================
		...
		\begin{alertbox}
			Quand on indique un \emph{nom} de fichier ou de dossier sans plus de précision,
			il doit se trouver dans le \emph{dossier courant}.
			Le shell ne va \emph{pas chercher} ailleurs ou dans un sous-dossier 
			pour le trouver. 
		\end{alertbox}
		...
		\begin{theorie}{Une première approche des chemins}
			En première approche, un \emph{chemin}
	...
	%====================
	\section{Déplacement dans une arborescence de fichiers}
	%=====================
		...
		\begin{Experience}{Se déplacer}  
			...
			\item 
				...
				\begin{itemize}
				\item 
					Cette commande demande de se \emph{déplacer}
					dans le dossier \textit{bin}.
					Elle \emph{change le dossier courant}.
				\end{itemize}
			\item 
		...
		\begin{theorie}{Se déplacer}
			...
			\item \kbd{cd} (\emph{change directory}) : permet de changer de dossier courant.
				...
				\item \kbd{cd ..}
					...
					On parle de \emph{répertoire parent}.
				\item \kbd{cd}
		...
		\begin{Exercice}{Assimiler les commandes de base}
			...
			\begin{enumerate}
				...
				\item Vous déplacer dans le dossier \samp{dev1} ;
...
%====================
	...
	\begin{Exercice}{Pour aller plus loin\dots}
		...
		\begin{enumerate}
		\item 
			Comment créer un fichier dont le nom contient une espace ?
		\item 
