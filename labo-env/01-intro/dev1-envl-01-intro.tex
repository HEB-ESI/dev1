...
%=====================
	...
	Faisons quelques expériences pour comprendre les principes de base des commandes.

	\begin{Tutoriel}{Ma première commande}
		\vspace{-1em}
		\begin{steps}
		\item Entrez la commande \kbd{ls}, (n'oubliez pas la touche \verb|ENTREE|).
		\end{steps}				
		Vous constatez que le bash a affiché quelque chose%
		\footnote{%
			D'incompréhensible pour le moment ; ne vous inquiétez pas nous y reviendrons
		}
		et qu'il vous propose à nouveau l'invite de commande.
	\end{Tutoriel}				

\newpage

	\begin{Tutoriel}{La casse} 
		\vspace{-1em}
		\begin{steps}
		\item Entrez à présent  la commande \kbd{LS}.
		\end{steps}
	\end{Tutoriel}	

	Vous voyez que le résultat est différent : 
	il ne comprend pas ce que vous lui voulez.
	En Linux, 
	\textbf{les majuscules et les minuscules n'ont pas le même sens, 
	vous devez respecter la casse.}
        
	\begin{Tutoriel}{Les espaces} 
		Tapez les 3 commandes suivantes qui ne se différencient 
	...
	%=====================
		...
		Simplement avec la commande \kbd{ls} que vous avez déjà rencontrée.
				
		\begin{Tutoriel}{Expérimentation}  
			\vspace{-1em}
			\begin{steps}
			\item Tapez la commande \kbd{ls}.
				\begin{itemize}
				...
				\end{itemize}
			\item Tapez à présent la commande \kbd{ls bin}.
				\begin{itemize}
				...
				\end{itemize}
			\item À présent, tapez la commande \kbd{cd bin}.
				\begin{itemize}
				\item Cette commande demande de se \textit{\textbf{déplacer}} 
					dans le dossier \textit{bin}.
				\end{itemize}
			\item Retapez la commande \kbd{ls} du début.
				\begin{itemize}
		...
		\begin{theorie}{Quelques commandes}
			\begin{itemize}
			\item \kbd{ls} : liste le contenu d'un dossier.
				\begin{itemize}
					\item \kbd{ls}
						(sans rien derrière) liste le contenu du dossier courant.
					\item \kbd{ls \emph{nom-dossier}}
						liste le contenu du dossier indiqué.
				\end{itemize}
			\item \kbd{cd} (\textit{change directory}) :
				permet de changer de dossier courant.
				\begin{itemize}
					\item \kbd{cd \emph{nom-dossier}}
		...
		\begin{Exercice}{Assimiler les commandes de base}
			...
			\begin{enumerate}
				...
				\item Vous déplacer dans le fichier \samp{bin} ;
		...
		\begin{Exercice}{cd vs ls}
			...
			\begin{enumerate}
				\item La commande \kbd{ls bin} ;
				\item La commande \kbd{cd bin} suivie de \kbd{ls} ?
			\end{enumerate}
	...
	%=====================
		...
		\begin{Tutoriel}{Permiers pas avec nano}
			\vspace{-1em}
			\begin{steps}
			\item En préambule, tapez la commande \kbd{cd} pour revenir dans \textit{votre home}.
			\item Tapez \kbd{nano test} pour commencer à éditer le fichier \samp{test} 
			...
			\item Tapez à présent la commande \kbd{ls}.
...
\newpage
	...
	%==================================
		...
		\begin{Exercice}{Utilisation d'un chemin}
			...
			\begin{enumerate}
			\item Afficher le contenu du fichier \samp{test}
...
%====================
	...
	\begin{itemize}
		\item Ce que font les commandes : 
			...
			\kbd{nano}, \kbd{rm}, \kbd{rmdir} et \kbd{touch}.
		\item Savoir éditer un petit texte avec \samp{nano}.
