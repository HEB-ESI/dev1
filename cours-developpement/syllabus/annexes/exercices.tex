...
\begin{Exercice}{Calcul de vitesse}
	...
	\begin{pseudocode}[1]
		\Algo{vitesseKMH}{\Par{distanceM, duréeS}{real}}{real}
		\Decl{distanceKM, duréeH}{real}
...
\begin{Exercice}{Les prix ronds}
	...
	\begin{pseudocode}
		\Algo{versPrixRond}{\Par{prixCentimes}{integer}}{integer}
...
\end{Emphase}
		...
		\begin{Exercice}{Nombre multiple de 5}
			...
			Ce qui donne~:
			\begin{pseudocode}[1]
				\Algo{multiple5}{\Par{nombre}{integer}}{boolean}
...
\begin{Exercice}{Compréhension}
	...
	\begin{pseudocode}
			\Algo{exerciceA}{a, b~: integer}{integer}
			\Decl{c}{integer}
			\Let c \Gets 2 * a
			\If{c > b}
			\Let c \Gets c - b
...
\end{multicols}	
	...
	\begin{Exercice}{Simplification d’algorithmes}
		...
		syntaxe mais contenant des lourdeurs d’écriture.  Simplifiez-les.
		Si vous avez l'impression que la condition est inutile, précisez-le.

	\begin{pseudocode}
		\Decl{estPair}{boolean} \Gets true 
		\If{estPair == true}
			\Write number
		\EndIf
	\end{pseudocode}
	...
	\begin{pseudocode}
		\Decl{estPair}{boolean} \Gets true 
		\If{estPair = true OR estPair == false}
			\Write number
		\EndIf
	\end{pseudocode}
	...
	\begin{pseudocode}
		\Decl{estPair}{boolean} \Gets true 
		\If{estPair == true ET estPair == false}
			\Write number
		\EndIf
	\end{pseudocode}
\end{Exercice}
...
\begin{Exercice}{Compréhension}
	...
	\begin{pseudocode}
		\Algo{exerciceB}{b, a : integer}{integer}
			\Decl{c}{integer}
			\If{a > b}
				\Let c \Gets a DIV b
			\Else
	...
	\end{pseudocode}
		...
		\begin{pseudocode}
			\Algo{exerciceC}{x1, x2~: integer}{integer}
			\Decl{estPlusGrand}{boolean}
			\Let estPlusGrand \Gets x1 > x2
			\If{estPlusGrand}
			\Let estPlusGrand \Gets estPlusGrand AND x1 == 4
			\Else
			\Let estPlusGrand \Gets estPlusGrand OR x2 == 3
		\EndIf
		\If{estPlusGrand}
		\Let x1 \Gets x1 * 1000
	\EndIf
...
\EndAlgo
	...
	\begin{Exercice}{Simplification d’algorithmes}
		...
		Voici quelques extraits d’algorithmes que vous pouvez supposer corrects
...
\section{Exercices~: modules et méthodes}
		...
		\begin{Exercice}{Tracer des algorithmes}
			...
			\Algo{ex1}{}{}
				\Decl{x, y}{integer}
				\Let x \Gets addition(3, 4)
				\Write x
				\Let x \Gets 3
				\Let y \Gets 5
				\Let y \Gets addition(x, y)
				\Write y
			\EndAlgo
			\Empty
			\Algo{addition}{a\In, b\In}{integer}
				\Decl{somme}{integer}
				\Let somme \Gets a + b	
				\Return somme
			\EndAlgo
			...
			\Algo{ex2}{}{}
				\Decl{a, b}{integer}
				\Let a \Gets addition(3, 4) \Comment voir ci-dessus
				\Write a
				\Let a \Gets 3
				\Let b \Gets 5
				\Let b \Gets soustraction(b, a)
				\Write b
			\EndAlgo
			\Empty
			\Algo{soustraction}{a\In, b\In}{integer}
				\Let c \Gets a - b
			\EndAlgo
			\end{pseudocode}
	...
	\begin{Exercice}{Appels de module}
		...
		\begin{pseudocode}
			\Entete{pgcd}{a\In,b\In~:~integer}{integer}
		\end{pseudocode}
...
\ 
		\begin{minipage}[t]{.5\linewidth}
			...
			\Decl{isDone}{boolean}
			\Decl{x}{integer}
			\Let isDone \Gets true
			\Let x \Gets 5
			\While{isDone}
			\Let x \Gets x + 7	
			\Let isDone \Gets x MOD 11 $\neq$ 0	
			\EndWhile
			\Write x
			\EndAlgo
			\end{pseudocode}
	...
	\begin{pseudocode}
		...
		\Let cpt \Gets 0
		\Let isDone \Gets true
		\While{isDone ET cpt < 3}
			\If{x MOD 2 ==  0}
				\Let x \Gets x + 1
				\Let isDone \Gets x < 20
			\Else
				\Let x \Gets x + 3
				\Let cpt \Gets cpt + 1
			\EndIf
		...
		\begin{Exercice}{Compréhension d’algorithmes}
			...
			des algorithmes suivants~?
				...
				\begin{pseudocode}
					...
					\Decl{isDone}{boolean}
					\Let x \Gets 3
					\Let isDone \Gets vrai
					\For{i}{1}{5}
					\Let x \Gets x + i
					\Let isDone \Gets isDone ET (x MOD 2 = 0)
				\EndFor
				\If{isDone}
				\Write x
				\Else
		...
		\begin{pseudocode}
			\Algo{boucle4}{}{}
				...
				\Repeat
					...
					\Let isBig \Gets x > 15
				\EndRepeat{NON isOdd AND NON isBig}
				\Write x
			\EndAlgo
\end{pseudocode}		
...
\bigskip
		...
		\begin{Exercice}{Lire un nombre}
			...
			\begin{pseudocode}
				\Algo{readNumber}{\Par{n}{integer}}{integer}
				\Decl{val}{integer}
				\Read{val}
				\Return val
			\EndAlgo
		...
		Pour tenir compte des entrées invalides, il faut ajouter une boucle qui
		prévient que la valeur est mauvaise et la redemande.  Et ceci, tant que
		c’est incorrect.  Ce qui donne~:
		
		\begin{pseudocode}
			\Algo{readNumber}{\Par{n}{integer}}{integer}
			\Decl{val}{integer}
			\Read{val}
			\While{val<1 OR val>n}
...
Dans ce cas, la valeur sentinelle est toute valeur correcte.
	...
	\begin{Exercice}{Lancé de dés}
		...
		\begin{pseudocode}
			\Entete{rollDice}{\Par{n}{integer}, \Par{val}{integer}}{integer}
		\end{pseudocode}
	\end{Exercice}
