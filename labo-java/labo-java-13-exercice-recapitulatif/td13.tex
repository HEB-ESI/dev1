...
Dans ce TD, on vous propose de mettre en pratique les notions vues auparavant pour créer un programme gérant l'enregistrement de nouveaux clients d'une entreprise (imaginaire). De nombreuses données différentes peuvent être associées à un client d'une entreprise donnée, mais pour simplifier les choses dans ce TD, nous considérerons uniquement les données suivantes : un identifiant, un nom, un prénom et une adresse e-mail. Tous ces champs sont enregistr\'es sous forme de cha\^ines de caract\`eres (des \path{String}). Nous considérerons qu'un identifiant client est simplement une suite de $10$ chiffres (choisis parmi $\{0,1, \ldots, 9\}$).%%FIXME: ou plus simplement "chiffres décimaux"
...
\vspace{0.3cm}
	...
	\end{Exercice} 
 	...
 	\begin{Exercice}{Cr\'eer un tableau de clients}
          %%FIXME: quel est le but de l'exercice, en dehors de copier coller le code donné ?
...
\end{Exercice}	
 ...
 \begin{Exercice}{Afficher le menu}
   Compl\'eter la m\'ethode \path{main} pour qu'elle affiche le menu d\'ecrit dans l'introduction. Le menu s'affiche \`a nouveau tant que l'utilisateur n'a pas choisi l'option quitter (choix num\'ero $4$).
   %%FIXME: je propose de donner une aide du type "Ecrivez une méthode qui s'occupe d'afficher le menu et recueillir le choix posé". Eventuellement proposer la signature de ladite méthode.
...
qui demande un identifant, un nom, un pr\'enom et une adresse e-mail \`a l'utilisateur et ajoute un nouveau client ayant exactement ces données apr\`es la derni\`ere case utilis\'ee du tableau \path{clients}.
				%%FIXME: proposer d'ajouter une méthode nbClients ?

Si toutes les cases du tableau sont d\'eja utilis\'ees, la m\'ethode renverra une \\\path{UnsupportedOperationException}. Compl\'eter la m\'ethode \path{main} (\`a vous de prendre garde à ne pas permettre \`a l'utilisateur de rentrer plus de \path{MAX_CLIENTS} clients, grâce à un message d'erreur.)
\end{Exercice}
...
\begin{Exercice}{V\'erifier le format de l'adresse e-mail}
		L'utilisateur peut se tromper en entrant les donn\'ees du client. Dans notre exemple imaginaire on va simplement vérifier que l'adresse contient exactement un symbole \texttt{@} suivi d'au moins un point, c'est-à-dire a la forme suivante:
		
                \begin{center}
                  \path{s1@s2.s3}
                \end{center}
		
		o\`u s1,s2,s3 sont des cha\^ines de caract\`ere ne contenant pas de \texttt{@}. Écrire une m\'ethode 
		
		\begin{Code}{Java}
		boolean verifierEmail(String email)
		\end{Code}
		
		qui renvoie \path{true} si email a le format d'une adresse e-mail et \path{false} sinon. (Dans la vraie vie, des librairies externes font cela bien mieux.)
...
\begin{Exercice}{V\'erifier l'absence de doublons}
		Bien que ce soit rare, il est possible que deux clients aient le m\^eme nom et le m\^eme pr\'enom. Par contre, ils ne peuvent avoir le m\^eme identifiant au sein de la soci\'et\'e. Écrire une m\'ethode
...
%%FIXME: je propose de ne pas imposer la méthode pour supprimer le client, simplement rappeler qu'il faut que juste mtetre à null ne suffit pas : il faut que tous les clients soient groupés en début de tableau, sans trou.
...
Mettre la m\'ethode \path{main} a jour. A vous de prendre garde \`a ce que l'utilisateur n'essaie pas de supprimer un client inexistant.
	%%FIXME: qu'est-ce qui est attendu ? try/catch ?
\end{Exercice}
